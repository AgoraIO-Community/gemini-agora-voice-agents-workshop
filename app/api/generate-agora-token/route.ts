...
import { agoraAppId, agoraAppCertificate } from '@/lib/env';
...
// Demo client: RTC + RTM token for the browser participant.
export async function GET(request: NextRequest) {
  const APP_ID = agoraAppId();
  const APP_CERTIFICATE = agoraAppCertificate();
  ...
  try {
    ...
    return NextResponse.json({ appId: APP_ID, token, uid: uid.toString(), channel: channelName });
  } catch (error) {
    console.error('Error generating Agora token:', error);
    return NextResponse.json(
      { error: 'Failed to generate Agora token', details: error instanceof Error ? error.message : String(error) },
      { status: 500 },
    );
  }
}
