...
export default function ConversationComponent({
  ...
  // ENABLE_AUDIO_PTS_METADATA is a module-level SDK parameter (not on the client instance).
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('Could not set ENABLE_AUDIO_PTS_METADATA:', error);
    }
  }, [client]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // The cascaded pipeline reports ASR latency only in the debug log stream.
        if (architecture === 'cascaded') {
          ai.on(AgoraVoiceAIEvents.DEBUG_LOG, (message) => {
            ...
            try {
              ...
              if (
                payload.object === 'message.metrics' &&
                payload.module === 'asr' &&
                ...
                setAgentMetrics((prev) => [
                  ...
                  {
                    type: 'asr',
                    name: payload.metric_name || 'latency',
                    value: payload.latency_ms,
                    timestamp: payload.send_ts || Date.now(),
                  },
                ].slice(-8));
              }
            } catch {}
  ...
  return (
    <QuickstartConversationLayout
      ...
      pipelineMetrics={<QuickstartPipelineMetrics metrics={agentMetrics} architecture={architecture} />}
      transcriptPanel={
