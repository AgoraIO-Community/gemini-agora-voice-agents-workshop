...
import { DEMO_MODELS, type DemoModel, type ThinkingLevel } from './ModelChoice';
import type { Architecture } from '@/lib/demo';
...
type LandingPageProps = {
  // Preselected by the deck (host controls) or the ?architecture= query.
  initialArchitecture: Architecture;
  // true when framed inside the deck: hides the footer, tightens spacing.
  embed?: boolean;
};

export default function LandingPage({ initialArchitecture, embed = false }: LandingPageProps) {
      ...
      const [agentData, rtm] = await Promise.all([
        ...
        fetch('/api/invite-agent', {
          ...
          body: JSON.stringify({
            ...
            architecture,
            ...(architecture === 'mllm' ? { model } : {}),
            ...(architecture === 'mllm' && model === DEMO_MODELS.extendedThinking ? { thinking_level: thinkingLevel } : {}),
    ...
    if (agoraData?.agentId) {
      try {
        ...
        const response = await fetch('/api/stop-conversation', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ agent_id: agoraData.agentId, architecture }),
        });
        if (!response.ok) {
  ...
  return (
    <div className="relative flex h-dvh min-h-screen flex-col overflow-hidden bg-background text-foreground">
      ...
      {/* Persistent attribution footer for the pre-call and in-call views (hidden when framed by the deck). */}
      {!embed && <footer className="fixed bottom-0 right-0 z-40 py-4 pr-4 md:py-6 md:pr-6">
